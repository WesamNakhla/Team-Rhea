...
class CommandFrame(tk.Frame, CommandFrameLogic):
    ...
    def init_ui(self):
        ...
        self.save_button.grid(row=4, column=0, pady=10, padx=10, sticky="e")
        
        self.exit_button = ttk.Button(self.main_frame, text="Cancel", command=self.app.switch_to_workspace_frame)
        self.exit_button.grid(row=4, column=0, pady=10, padx=10, sticky="w")

    def add_form_ui(self):
        self.new_command_frame = tk.Frame(self.main_frame)
        self.new_command_frame.grid(row=1, column=0, pady=10, sticky="ew")
        self.new_command_frame.grid_columnconfigure(0, weight=1)
        self.new_command_frame.grid_columnconfigure(1, weight=1)
        self.new_command_frame.grid_columnconfigure(2, weight=1)
        
        self.new_command_label = tk.Label(self.new_command_frame, text="Enter command", font=('Arial', 12))
        self.new_command_label.grid(row=0, column=0, padx=5, pady=5, sticky="ew")
        
        self.new_type_label = tk.Label(self.new_command_frame, text="Enter type", font=('Arial', 12))
        self.new_type_label.grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        
        self.new_description_label = tk.Label(self.new_command_frame, text="Enter description", font=('Arial', 12))
    ...
    def create_command_row(self, index, command):
        command_entry = ttk.Entry(self.command_container, width=20)
        ...
        type_entry = ttk.Entry(self.command_container, width=20)
        ...
        description_entry = ttk.Entry(self.command_container, width=30)
    ...
    def add_command_ui(self):
        command = {
            "command": self.new_command_label.cget("text"),
            "type": self.new_type_label.cget("text"),
            "description": self.new_description_label.cget("text")
        }
