...
class ModulesOutputFrame(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)
        ...
        self.columns = ["Offset", "Base", "Size", "Name", "Path", "File output"]
        ...
        # Add buttons for exporting to CSV and showing the bar chart
        self.export_button = ttk.Button(self, text="Export to CSV", command=self.export_to_csv)
        self.export_button.grid(row=2, column=0, padx=10, pady=5, sticky="ew")
    ...
    def sort_treeview(self, col, reverse):
        def convert(data):
            try:
                return int(data, 16) if '0x' in data else int(data)
            except ValueError:
                try:
                    return float(data)
                except ValueError:
    ...
    def show_bar_chart(self):
        sizes = [int(self.tree.item(item, "values")[2], 16) for item in self.tree.get_children()]
        names = [self.tree.item(item, "values")[3] for item in self.tree.get_children()]

        fig = Figure(figsize=(10, 6), dpi=100)
        ax = fig.add_subplot(111)
        ax.bar(names, sizes, color='blue')
        ax.set_xlabel('Kernel Module Name')
        ax.set_ylabel('Size (Bytes)')
        ax.set_title('Kernel Modules Sizes')
        ax.tick_params(axis='x', rotation=45)

        chart_window = tk.Toplevel(self)
        chart_window.title("Kernel Module Size Bar Chart")
        canvas = FigureCanvasTkAgg(fig, master=chart_window)
        canvas.draw()
        canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)

class HandlesOutputFrame(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)
        ...
        self.columns = ["PID", "Handle Value", "Object Type", "Name"]
        ...
        self.context_menu.add_command(label="Copy", command=self.copy_selected)
        self.context_menu.add_command(label="Export to CSV", command=self.export_to_csv)
        self.context_menu.add_command(label="Show Pie Chart", command=self.show_pie_chart)
        
        self.export_button = ttk.Button(self, text="Export to CSV", command=self.export_to_csv)
        self.export_button.grid(row=2, column=0, padx=10, pady=5, sticky="ew")
        
        self.chart_button = ttk.Button(self, text="Show Pie Chart", command=self.show_pie_chart)
        self.chart_button.grid(row=2, column=1, padx=10, pady=5, sticky="ew")
    ...
    def populate(self, findings):
        ...
        for line in filtered_lines:
            values = re.split(r'\s+', line.strip(), maxsplit=3)
            values = self.adjust_columns(values)
            if len(values) == len(self.columns):
                self.tree.insert("", "end", values=values)
    ...
    def show_pie_chart(self):
        handle_types = [self.tree.item(item, "values")[2] for item in self.tree.get_children()]
        counter = Counter(handle_types)
        ...
        ax = fig.add_subplot(111)
        ax.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=140)
        ax.set_title('Handle Types Distribution')
        
        chart_window = tk.Toplevel(self)
        chart_window.title("Handle Types Pie Chart")
        canvas = FigureCanvasTkAgg(fig, master=chart_window)
        canvas.draw()
        canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
